...
def render_timesheet(tax_rate: float) -> None:
    ...
    with upload_col:
        ...
        if ts_upload is not None:
            if f"_ts_ingested_{ts_upload.name}_{ts_upload.size}" not in st.session_state:
                with st.spinner(f"Vaulting {ts_upload.name}..."):
                    was_new, msg = ingest_to_vault(ts_upload)
                st.session_state[f"_ts_ingested_{ts_upload.name}_{ts_upload.size}"] = True
                if was_new:
                    st.success(msg)
                    _status(f"Timesheet file ingested: {msg}")
                    st.rerun()
                else:
                    st.info(msg)
